...
3. **Playground** (optional) — a component in `components/playgrounds/` registered in `playgrounds/index.ts`. When present, the player shows a Walkthrough / Playground toggle. `bridge`, `threads`, and `jsi` have one so far.
...
`bridge` (flagship, playground), `threads` (playground), `jsi` (playground), `lifecycle`, `usestate`, `useeffect`, `flatlist`, `hermes`, `metro`, `navigation`, `animated`.
